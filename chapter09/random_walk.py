...
class Tiles3ValueFunction:
    ...
    def __init__(self, iht, num_tilings):
        ...
        # tilingSize is the number of tiles in one tiling.
        # self.tilingSize = N_STATES // tileWidth + 1
        self.num_of_tiles_per_tiling = self.iht.size // num_tilings   # 1024 / 64 = 16
        # weight for each tile
        self.params = np.zeros(self.iht.size)
    ...
    def value(self, state):
        ...
        for tiling_index in range(0, self.num_of_tilings):
            tile_indices = tiles(self.iht, self.num_of_tilings, state)
            stateValue += self.params[tile_indices[tiling_index]]
    ...
    def update(self, delta, state):
        ...
        for tiling_index in range(0, self.num_of_tilings):
            ...
            self.params[tile_indices[tiling_index]] += delta
...
def gradient_monte_carlo(value_function, alpha, distribution=None):
    ...
    for state in trajectory[:-1]:
        if value_function == Tiles3ValueFunction:     # because the state type in tiles3 is list
            state = [state]
...
# gradient Monte Carlo algorithm for tiles3.py
# @value_function: an instance of class ValueFunction
# @alpha: step size
# @distribution: array to store the distribution statistics
def gradient_monte_carlo_tiles3(value_function, alpha, distribution=None):
    state = START_STATE
    trajectory = [state]
    ...
    # Gradient update for each state in this trajectory
    for state in trajectory[:-1]:
        #state = [state]     # because the state type in tiles3 should be list
        delta = alpha * (reward - value_function.value([state]))
        value_function.update(delta, [state])
        if distribution is not None:
            distribution[state] += 1
...
def figure_9_11(true_value):
    ...
    labels = ['tile coding by tiles3.py (64 tilings)', 'state aggregation (one tiling)']
    iht = IHT(1024)
    num_tilings = 64
    floats = 500 # just for initial the list of value_functions, doesn't matter
    # ints = None
    # track errors for each episode
    errors = np.zeros((len(labels), episodes))
    for run in range(runs):
        # initialize value functions for multiple tilings and single tiling
        value_functions = [Tiles3ValueFunction(iht, num_tilings),
                         ValueFunction(N_STATES // tile_width)]
        for i in range(len(value_functions)):
            for episode in tqdm(range(episodes)):
                # I use a changing alpha according to the episode instead of a small fixed alpha
                ...
                # gradient Monte Carlo algorithm
                if i == 0:
                    gradient_monte_carlo_tiles3(value_functions[i], alpha)
                    state_values = [value_functions[i].value([state]) for state in STATES]
                    # get the root-mean-squared error
                    errors[i][episode] += np.sqrt(np.mean(np.power(true_value[1: -1] - state_values, 2)))
                else:
    ...
    plt.savefig('../images/figure_9_11.png')
    plt.close()
